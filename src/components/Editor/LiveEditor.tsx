	...
	async function connectToWebSocket() {
		try {
			...
			const socket = new WebSocket(
				`ws://localhost:5050/ws?token=${accessToken}`,
			);

			// authorize session
			const liveSession = await postRequest(
				"/livesession",
				{ documentId },
				true,
			);
			console.log("@live liveSession response:", liveSession);
			...
			console.log("Error occured while attempting to create live session.");
		}
	}
